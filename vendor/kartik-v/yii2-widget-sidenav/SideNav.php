...
class SideNav extends \yii\widgets\Menu
{
     ...
     */
    public $indMenuOpen = '<i class="indicator glyphicon glyphicon-chevron-down" style="font-size:9px;"></i>';

    /**
     * @var string indicator for a closed sub-menu
     */
    public $indMenuClose = '<i class="indicator glyphicon glyphicon-chevron-left" style="font-size:9px;"></i>';
    ...
    protected function renderItem($item)
    {
        ...
        if (empty($item['top'])) {
            if (empty($item['items'])) {
                ...
                $template = isset($item['template']) ? $item['template'] :'<a href="{url}" class="kv-toggle" ">{icon}<span><strong class="labels" >{label}</strong></span></a>';
