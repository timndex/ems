...
?>
   ...
   <h5><strong>Audit Report</strong></h5>
     

<p id="date_filter">
	 <div class="row">
 <div class="col-sm-3">
  <strong> Filter From</strong><input placeholder="Selected starting date" type="date" id="filter_From" class="form-control datepicker">
</div>
<div class="col-sm-3">
    <strong>Filter To </strong> <input placeholder="Selected starting date" type="date" id="filter_To" class="form-control datepicker">
 </div>
...
<table id="dataTables" style="width:100%"  class="table table-striped table-bordered" style="border: 1px solid black">
        <thead>
            <tr>
...
?>			
        		...
        		<td><?= date('m/d/yy', strtotime($created_at));?></td>
        		<?php
        		  $modelCustomers1 = Customers::find()->where(['customer_id'=>$customer_id])->all();
                foreach ($modelCustomers1 as  $val) {
                	$customer_names = $val['customer_names'];
                ?>
...
?>
               ...
               // }
                ...
                <?php
        		  $modelCustomers2 = Customers::find()->where(['customer_id'=>$customer_id])->all();
                foreach ($modelCustomers2 as  $vals) {
                	$customernames = $vals['customer_names'];
                ?>
        		<td><?= $customernames;?></td>
        		<?php
        		}
...
<tfoot>
            <tr>
                <th colspan="6" style="text-align:right">Total (Ksh)</th>
                <th id='Total'></th>
                <th id='Total1'></th>
                <th id='Total2'></th>
                <th id='Total3'></th>
